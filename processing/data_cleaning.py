...
def clean_dataset(df: pd.DataFrame, start_date: str = None, end_date: str = None) -> pd.DataFrame:
    ...
    # Incohérence Météo : DNI ou DHI supérieur au GHI
    tolerance = 1e-6
    invalid_meteo = ((df_clean["dni"] > df_clean["ghi"] + tolerance) | 
                     (df_clean["dhi"] > df_clean["ghi"] + tolerance)).sum()
    # On supprime ou corrige. On choisit de supprimer les lignes physiquement impossibles.
    df_clean = df_clean[~((df_clean["dni"] > df_clean["ghi"] + tolerance) | 
                          (df_clean["dhi"] > df_clean["ghi"] + tolerance))]
    logger.info("Suppression des incohérences DNI/DHI > GHI : %d lignes supprimées", invalid_meteo)
